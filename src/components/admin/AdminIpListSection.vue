  ...
  <section class="mt-8">
    <div class="mb-4 flex items-center justify-between">
      <h2 class="flex items-center gap-2 text-lg font-bold text-light-text dark:text-dark-text">
        ...
        IP {{ ipListMode === 'whitelist' ? t('admin.whitelist', 'Whitelist') : t('admin.blacklist', 'Blacklist') }}
        <span
        ...
        >
          {{ ipListMode === 'whitelist' ? t('admin.whitelistMode', 'Whitelist Mode') : t('admin.blacklistMode', 'Blacklist Mode') }}
        </span>
      </h2>
      <div class="flex items-center gap-2">
        <button class="btn-secondary flex items-center gap-1.5 text-sm" @click="$emit('toggleMode')">
          ...
          {{ ipListMode === 'whitelist' ? t('admin.switchToBlacklist', 'Switch to Blacklist') : t('admin.switchToWhitelist', 'Switch to Whitelist') }}
        </button>
    ...
    <div class="card overflow-hidden">
      <div v-if="entries.length === 0" class="py-8 text-center text-sm text-gray-400">
        {{ t('admin.noIpEntries', 'No IP entries') }}
      </div>
      ...
      <div v-else>
        <div class="grid grid-cols-12 gap-2 border-b border-light-border px-4 py-2 text-xs font-medium text-gray-500 dark:border-dark-border dark:text-dark-text-secondary">
          ...
          <div class="col-span-3">{{ t('admin.reason', 'Reason') }}</div>
          <div class="col-span-2 hidden sm:block">{{ t('admin.createdBy', 'Created By') }}</div>
          <div class="col-span-2 hidden md:block">{{ t('admin.createdAt', 'Created At') }}</div>
          <div class="col-span-1 text-right">{{ t('common.actions', 'Actions') }}</div>
        </div>
