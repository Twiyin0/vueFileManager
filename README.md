...
- 桌面端聚焦搜索（`Ctrl+K`）支持按 `Esc` 或点击遮罩关闭，会过滤 `.trash` 路径，并在跳转到结果所在目录后让对应项目高亮闪烁三次
- 中等列表视图，支持基于服务端缩略图缓存的懒加载视频缩略图与自适应列表高度
- 跨存储池复制与移动
- 跨池共享挂载，可将多个存储池目录统一挂载到 `/share`
- 远程上传与后台离线下载任务，支持逗号、竖线或一行一个的多链接批量提交
...
- 输入框支持一个或多个远程链接，多个链接之间可使用逗号、竖线或换行分隔
...
请先把 WAL 日志合并回主数据库，再切换到当前运行时。若执行 `PRAGMA wal_checkpoint(TRUNCATE);` 后仅残留一个 0 字节或极小的占位 `*.db-wal` 文件，当前运行时会自动忽略并清理该占位文件。
