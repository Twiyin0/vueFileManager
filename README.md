...
### 插件系统
- 📦 插件目录 `plugins/`，每个插件含 `manifest.json`
- 🎨 主题插件：自定义 CSS 样式，自动注入页面
- 📌 Hook 插件：扩展 Express 后端（中间件/路由）
- 💾 存储插件：自定义存储驱动（实现 StorageProvider 接口）
