...
重命名文件或目录。
请求体中的 `newName` 仅应传入新名称本身，不要传完整目标路径。
对于不支持原生目录重命名的存储后端，服务端会自动退化为递归树操作。

### `POST /api/files/move`

在同一存储池内移动文件或目录。
对于不支持原生目录移动的存储后端，服务端会自动退化为递归树操作。

### `POST /api/files/copy`

在同一存储池内复制文件或目录。
对于不支持原生目录复制的存储后端，服务端会自动退化为递归树操作。
...
- 如果预检查阶段已经拿到远程源站的 `4xx` 响应，这类明显无效的链接会直接跳过，不会创建后台任务，并在 `errors` 中返回原因
